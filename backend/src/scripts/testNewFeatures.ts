...
async function testNewFeatures() {
  ...
  try {
    ...
    // Create an employee
    const empRes = await axios.post(`${BASE_URL}/users`, {
  ...
  try {
    const res = await axios.post(`${BASE_URL}/workflows/managers/assign`, {
      employeeId,
      managerId,
    });
  ...
  try {
    const res = await axios.get(`${BASE_URL}/workflows/managers/${managerId}/team`);
  ...
  try {
    const res = await axios.get(
      `${BASE_URL}/workflows/employees/${employeeId}/manager`
    );
    if (res.data.data) {
  ...
  let workflowId = 0;
  try {
    const res = await axios.post(`${BASE_URL}/workflows`, {
      company_id: 1,
  ...
  if (workflowId > 0) {
    try {
      const res = await axios.get(`${BASE_URL}/workflows/${workflowId}`);
  ...
  if (expenseId > 0 && workflowId > 0) {
    try {
      const res = await axios.get(
        `${BASE_URL}/workflows/expenses/${expenseId}/rules/${workflowId}`
      );
  ...
  if (expenseId > 0) {
    try {
      const res = await axios.get(
        `${BASE_URL}/workflows/expenses/${expenseId}/approval-status`
      );
  ...
  if (expenseId > 0) {
    try {
      ...
      await axios.post(`${BASE_URL}/expenses/${expenseId}/submit`);
      console.log(`✅ Submitted expense for approval`);

      // Move to next approver
      const res = await axios.post(
        `${BASE_URL}/workflows/expenses/${expenseId}/next-approver/${workflowId}`
      );
  ...
  if (expenseId > 0) {
    try {
      const res = await axios.post(
        `${BASE_URL}/approvals/${expenseId}/approve`,
        {
