...
async function runTests() {
    ...
    await test("Get specific user by ID", async () => {
        const res = await axios_1.default.get(`${BASE_URL}/users/${userId}`);
    ...
    await test("Get specific expense by ID", async () => {
        const res = await axios_1.default.get(`${BASE_URL}/expenses/${expenseId}`);
    ...
    await test("Submit expense for approval", async () => {
        const res = await axios_1.default.post(`${BASE_URL}/expenses/${expenseId}/submit`);
        if (!res.data.success)
    ...
    await test("Admin can access all users", async () => {
        ...
        if (!Array.isArray(res.data.data))
            throw new Error("Failed to get users as admin");
    ...
    await test("Admin can view all expenses", async () => {
        ...
        const res = await axios_1.default.get(`${BASE_URL}/expenses`);
        if (!Array.isArray(res.data.data))
            throw new Error("Failed to get expenses as admin");
    ...
    await test("Audit logs are created for operations", async () => {
        try {
            const res = await axios_1.default.get(`${BASE_URL}/audit-logs`);
            if (Array.isArray(res.data.data) && res.data.data.length > 0) {
