...
async function testDataFetch() {
    try {
        ...
        users.forEach((u) => console.log(`   - ID: ${u.id}, Email: ${u.email}, Role: ${u.role.name}`));
        // Test Expenses
        const expenses = await prisma.expense.findMany({
            include: { employee: true },
        });
        console.log(`\n✅ Expenses: ${expenses.length} found`);
        expenses.forEach((e) => console.log(`   - ID: ${e.id}, Amount: ${e.amount} ${e.currency}, Status: ${e.status}`));
        // Test Approvals
        const approvals = await prisma.expenseApproval.findMany({
            include: { expense: true, approver: true },
        });
        console.log(`\n✅ Approvals: ${approvals.length} found`);
        approvals.forEach((a) => console.log(`   - ID: ${a.id}, Expense: ${a.expense.description}, Status: ${a.status}`));
        // Test Categories
...
testDataFetch();
//# sourceMappingURL=testDataFetch.js.map